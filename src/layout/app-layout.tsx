...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['institute-owner', 'website-administrator', 'institute-manager', 'course-coordinator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<InstituteInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/institutes` : null),
    () => getInstitutes({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Institutes',
      path: isTenantUser ? `/institutes/view/${data?.[0]?.id}` : '/institutes',
      entity: 'institute',
