...
import { getEnquiries, deleteEnquiryById } from 'apiSdk/enquiries';
import { EnquiryInterface } from 'interfaces/enquiry';

type ColumnType = ColumnDef<EnquiryInterface, unknown>;
...
export function EnquiryListPage(props: EnquiryListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EnquiryInterface>>(
    () => `/enquiries?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EnquiryInterface) => {
    if (hasAccess('enquiry', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/enquiries/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/enquiries/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('enquiry', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/enquiries/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('enquiry', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="enquiry" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Enquiries
        </Text>
        {hasAccess('enquiry', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/enquiries/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
