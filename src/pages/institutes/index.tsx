...
import { getInstitutes, deleteInstituteById } from 'apiSdk/institutes';
import { InstituteInterface } from 'interfaces/institute';

type ColumnType = ColumnDef<InstituteInterface, unknown>;

interface InstituteListPageProps {
...
export function InstituteListPage(props: InstituteListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getInstitutes({
        relations: ['user', 'course.count', 'service.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<InstituteInterface>>(
    () => `/institutes?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: InstituteInterface) => {
    if (hasAccess('institute', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/institutes/view/${row.id}`);
    }
  };
...

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/institutes/view/${record.id}`} passHref legacyBehavior>
            <Button
          ...
          {hasAccess('institute', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/institutes/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
          ...
          {hasAccess('institute', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
            <IconButton
              onClick={(e) => {
                e.stopPropagation();
  ...
  return (
    <Box p={4} rounded="md" shadow="none">
      <AccessInfo entity="institute" />
      <Flex justifyContent="space-between" mb={4}>
        <Text as="h1" fontSize="1.875rem" fontWeight="bold" color="base.content" {...titleProps}>
          Institutes
        </Text>
        {hasAccess('institute', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/institutes/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
